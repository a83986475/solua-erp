frappe.pages["a4-print-designer"].on_page_load = function (wrapper) {
	...
	const labels = {image:"FOTO",name:"Artigo / 商品",spu:"SPU",sku:"SKU / 货号",cor:"COR",barcode:"EAN / 条码",description:"Descrição / 描述",ordered:"Qt. pedido / 已订购",remaining:"Qt. restante / 剩余",qty:"Qt",picked:"Qt separado / 已拣",uom:"Un.",rate:"Prc",amount:"Valor / 金额",trace:"Rastreabilidade / 追溯",warehouse:"Armazém / 仓库",order:"S.O. / 订单"};
	const base = ["image","name","spu","sku","cor","barcode","description"];
	const columns = {"Sales Order":["qty","uom","rate","amount"],"Sales Invoice":["qty","uom","rate","amount"],"Delivery Note":["ordered","remaining","uom","rate","amount","trace"],"Pick List":["qty","picked","uom","warehouse","order"]};
	const defaults = {"Sales Order":{image:7,name:13,spu:8,sku:13,cor:8,barcode:12,description:18,qty:8,uom:6,rate:7,amount:8},"Sales Invoice":{image:7,name:13,spu:8,sku:13,cor:8,barcode:12,description:18,qty:8,uom:6,rate:7,amount:8},"Delivery Note":{image:6,name:10,spu:8,sku:10,cor:6,barcode:8,description:11,ordered:10,remaining:10,qty:10,uom:5,rate:5,amount:6,trace:5},"Pick List":{image:8,name:14,spu:8,sku:13,cor:8,barcode:12,description:17,qty:8,picked:8,uom:5,warehouse:8,order:7}};
	const config = {doctype:types[0],visible:{},widths:JSON.parse(JSON.stringify(defaults)),settings:{fontSize:9,titleSize:18,headSize:9,lineHeight:1.35,cellPadding:1.5,pageMargin:12,titleColor:"#99732c",headBg:"#f5f1e9"}};
	for (const type of types) config.visible[type]=Object.fromEntries([...base,...columns[type]].map(key=>[key,key!=="image"]));
	config.visible["Delivery Note"].qty=false;
	let preview = null, canSave = false;
	page.main.html(`<style>
.a4d{display:grid;grid-template-columns:330px 1fr;min-height:calc(100vh - 120px);color:#25313a}.a4d-panel{padding:18px;background:white;border-right:1px solid #ccd3d8;overflow:auto}.a4d-panel h2{font-size:15px;margin:12px 0}.a4d-field{display:grid;grid-template-columns:1fr 115px;align-items:center;gap:8px;margin:8px 0}.a4d-field input,.a4d-field select{width:100%}.a4d-checks{display:grid;grid-template-columns:1fr 1fr;gap:6px}.a4d-widths{display:grid;grid-template-columns:1fr 75px;gap:6px 10px;align-items:center}.a4d-widths input{width:75px}.a4d-group{border-top:1px solid #d7dde1;padding:10px 0}.a4d-actions{display:grid;gap:8px}.a4d-stage{background:#eef1f3;padding:20px;overflow:auto}.a4d-paper{width:210mm;min-height:297mm;margin:auto;padding:12mm;background:#fff;box-shadow:0 2px 18px #0002;box-sizing:border-box;color:#25313a;font-size:9pt;overflow-wrap:anywhere}.a4d-paper h1{font-size:18pt;color:#99732c;text-align:center}.a4d-paper table{width:100%;border-collapse:collapse;table-layout:fixed}.a4d-paper .a4d-items{font-size:9pt;line-height:1.35}.a4d-paper th,.a4d-paper td{padding:1.5mm;border:1px solid #aeb8be;vertical-align:top;overflow-wrap:anywhere}.a4d-paper th{background:#f5f1e9}.a4d-paper thead{display:table-header-group}.a4d-paper tr{break-inside:avoid}.a4d-photo{width:min(12mm,100%);aspect-ratio:1;object-fit:cover;display:block}.a4d-mock{width:min(12mm,100%);aspect-ratio:1;background:linear-gradient(135deg,#e8ecef,#aeb7bd);border:1px solid #c5ccd0;box-shadow:0 1mm 2mm #25313a40}.a4d-total{text-align:right;margin:3mm 0;font-size:11pt;font-weight:700}.a4d-status{padding:8px;background:#f6f7f8;white-space:pre-wrap}.a4d-meta{margin:10px 0}.a4d-meta td{width:50%}.a4d-tools{display:grid;grid-template-columns:1fr 1fr;gap:8px}.a4d-tools button{width:100%}@media(max-width:1000px){.a4d{grid-template-columns:1fr}.a4d-stage{padding:12px}.a4d-paper{margin:0}}
</style><div class="a4d"><aside class="a4d-panel"><p>使用真实 ERPNext 单据预览；格式只会另存为新打印格式。</p><div class="a4d-group"><h2>真实单据</h2><label class="a4d-field">单据类型<select data-role="doctype">${types.map(x=>`<option>${x}</option>`).join("")}</select></label><label class="a4d-field">单据<select data-role="document"><option value="">先选择单据类型</option></select></label><button class="btn btn-default" data-action="load-preview">加载真实预览</button></div><div class="a4d-group"><h2>版式</h2>${[["fontSize","字号",7,13,1],["titleSize","标题字号",12,26,1],["headSize","表头字号",6,14,1],["lineHeight","正文行高",1,2,.05],["cellPadding","单元格内边距",.5,3,.5],["pageMargin","页面内边距",5,20,1]].map(([key,label,min,max,step])=>`<label class="a4d-field">${label}<input data-setting="${key}" type="number" min="${min}" max="${max}" step="${step}" value="${config.settings[key]}"></label>`).join("")}<label class="a4d-field">标题颜色<input data-setting="titleColor" type="color" value="#99732c"></label><label class="a4d-field">表头底色<input data-setting="headBg" type="color" value="#f5f1e9"></label></div><div class="a4d-group"><h2>标题列（每列独立开关）</h2><div class="a4d-checks" data-role="checks"></div></div><div class="a4d-group"><h2>列宽（合计 100%）</h2><div class="a4d-widths" data-role="widths"></div><button class="btn btn-default" data-action="equal">均分可见列</button></div><div class="a4d-group"><h2>打开已保存设计</h2><label class="a4d-field">格式<select data-role="format"><option value="">选择格式</option></select></label><button class="btn btn-default" data-action="open-format">加载设计设置</button></div><div class="a4d-group"><h2>另存为打印格式</h2><label class="a4d-field">新格式名称<input data-role="format-name" maxlength="140" placeholder="例如：销售单 A4 测试"></label><button class="btn btn-primary" data-action="save">保存为新打印格式</button></div><div class="a4d-status" data-role="status" role="status" aria-live="polite">正在读取权限…</div></aside><main class="a4d-stage"><article class="a4d-paper" data-role="paper"><h1>Solua Home · A4</h1><p>选择单据并加载真实预览。</p></article></main></div>`);
	...
	const rowValue = (row,key) => ({name:row.item_name,sku:row.order_code||row.item_code,cor:row.color||row.color_code}[key] ?? row[key]);
	...
	function loadFormats(){call("list_formats",{doctype:config.doctype}).then(rows=>root.find('[data-role="format"]').html('<option value="">选择格式</option>'+rows.map(x=>`<option>${esc(x.name)}</option>`).join(""))).catch(()=>{});}
	root.on("change","[data-role=doctype]",function(){config.doctype=this.value;preview=null;render();loadDocuments();loadFormats();});
	...
	root.on("change","[data-setting]",function(){const key=this.dataset.setting;config.settings[key]=key.endsWith("Color")||key==="headBg"?this.value:Number(this.value);render();});
	root.on("click","[data-action]",async function(){
		const action=this.dataset.action;
		try{
			if(action==="load-preview") {const name=root.find('[data-role="document"]').val();if(!name)throw new Error("请选择真实单据");preview=await call("get_preview",{doctype:config.doctype,name});render();status(`已加载真实单据 ${preview.name}，${preview.items.length} 行。`);}
			if(action==="equal"){const cols=visibleCols();cols.forEach(k=>config.widths[config.doctype][k]=100/cols.length);render();}
			if(action==="open-format"){const name=root.find('[data-role="format"]').val();if(!name)throw new Error("请选择格式");const loaded=await call("load_config",{name});config.doctype=loaded.doctype;config.visible[loaded.doctype]=loaded.visible;config.widths[loaded.doctype]=loaded.widths;config.settings=loaded.settings;for(const key of Object.keys(config.settings))root.find(`[data-setting="${key}"]`).val(config.settings[key]);root.find('[data-role="doctype"]').val(config.doctype);preview=null;render();loadDocuments();loadFormats();status(`已加载 ${name} 的设计设置；请重新加载真实单据预览。`);}
			if(action==="save"){if(!canSave)throw new Error("当前账号没有创建打印格式的权限");if(!preview)throw new Error("请先加载一张真实单据");const name=root.find('[data-role="format-name"]').val().trim();if(!name)throw new Error("请输入新的格式名称");const saved=await call("save_format",{name,config:JSON.stringify({doctype:config.doctype,visible:config.visible[config.doctype],widths:config.widths[config.doctype],settings:config.settings}),sample_name:preview.name});status(`已创建 ${saved.name}。`);loadFormats();}
		}catch(e){status(e.message||"操作失败");frappe.msgprint({title:__("A4 打印设计器"),message:e.message||__("操作失败"),indicator:"red"});}
	});
	call("get_access").then(data=>{canSave=!!data.can_save;if(!canSave)root.find('[data-action=save]').prop("disabled",true);status(canSave?"可预览与新建打印格式。":"可预览；当前账号没有 Print Format 创建权限，保存已禁用。");}).catch(e=>status(e.message||"权限读取失败"));
