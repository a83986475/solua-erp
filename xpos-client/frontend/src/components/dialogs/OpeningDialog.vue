<template>
	...
	>
		<div class="w-full max-w-md animate-in fade-in zoom-in-95 duration-300 my-auto">
			...
			<div class="mb-4 flex items-center justify-between gap-3 rounded-lg bg-background/10 px-3 py-2">
				<div class="min-w-0 text-primary-foreground">
					<p class="text-[11px] uppercase tracking-wide text-primary-foreground/60">
						{{ __("Signed in as") }}
					</p>
					<p class="truncate text-sm font-semibold">
						{{ authStore.userFullName || authStore.userName }}
					</p>
				</div>
				<div class="flex shrink-0 items-center gap-2">
					<LanguageSelector />
					<Button variant="secondary" size="sm" class="gap-1.5" @click="authStore.logout()">
...
import { Loader2, Lock, LogOut, RefreshCw } from "lucide-vue-next";
