...
function normalizeItemImage<T extends Record<string, unknown>>(item: T): T {
	const normalized: Record<string, unknown> = { ...item };
	const imageValue = normalized["image"];
	const image = typeof imageValue === "string" ? imageValue.trim() : "";
	const swatch =
		typeof normalized["custom_swatch_image"] === "string"
			? (normalized["custom_swatch_image"] as string).trim()
			: "";
	if (!image && swatch) normalized["image"] = swatch;
	return normalized as T;
}

function normalizeItemImages(items: POSItem[]): POSItem[] {
	return items.map((item) => normalizeItemImage(item as unknown as Record<string, unknown>) as POSItem);
}
...
export const useItemStore = defineStore("items", () => {
	...
	async function cacheAllItems(posProfile: string): Promise<void> {
		...
		try {
			...
			do {
				...
				allItems = [...allItems, ...normalizeItemImages(batch)];
				start += batchSize;
	...
	async function fetchItems(posProfile: string, append = false): Promise<void> {
		...
		try {
			...
			if (isOnline()) {
				const result = normalizeItemImages(
					await call<POSItem[]>("xpos.api.items.get_pos_items", {
					pos_profile: posProfile,
	...
	async function fetchItemDetail(
		...
		try {
			const result = normalizeItemImage(
				await call<ItemDetail>("xpos.api.items.get_item_detail", {
				item_code: itemCode,
				pos_profile: posProfile,
				warehouse: warehouse || "",
				}),
			...
			if (!result.image) {
				const cached = await getCachedItemByCode(itemCode).catch(() => null);
				if (cached?.image) result.image = cached.image as string;
			}
